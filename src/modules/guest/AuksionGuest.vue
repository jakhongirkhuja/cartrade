            ...
            <template v-if="cars.length>0">
                <template v-for="car in cars">
                    <div class="aukstion__block--items" v-if="car.car" >
                        <router-link :to="{ name: 'main.auksion.inside', params: { id: car.id } }" class="poster" v-bind:style="{ 'background-image': 'url(' + url+'/files/cars/'+car.car.images[0].image + ')' }"></router-link>
                        <div class="info">
                            <h2 class="info__title"><router-link :to="{ name: 'main.auksion.inside', params: { id: car.id } }">{{car.car.car_mark.name}} {{car.car.car_model.name}} - {{car.car.year}}</router-link></h2>
                        </div>
                        <div class="actions">
